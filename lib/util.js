  ...
  var deps = _(fileDeps)
    .map(function(file) {
      if (isRelative(file)) {
        ...
        var fullpath = join(pkg.dest, file);
...
function resolvePath(file, base) {
  if (!isRelative(file) || !base) return file;
  debug('transport relative id(%s) of basepath(%s)', file, base);
  file = join(dirname(base), file);
  if (isRelative(file)) throw new PluginError('resolvePath', file + ' is out of bound');
  return file;
}
