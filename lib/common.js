...
function transportDeps(filepath, pkg, options) {
  if (!pkg.files[filepath]) {
    throw new PluginError('transportDeps', filepath + ' is not included in files:' + Object.keys(pkg.files));
  }
...
function getFile(gfile, pkg) {
  ...
  if (!file) {
    ...
    if (!hasFound) {
      ...
      throw new PluginError('getFile', 'not found ' + filepath + ' of pkg ' + pkg.id);
    }
  }
