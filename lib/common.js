...
function transportId(filepath, pkg, options) {
  ...
  if (isFunction(idleading)) {
    idleading = idleading(filepath, pkg);
  }
...
function getFileInfo(file, pkg) {
  ...
  if (!pkg.files[filepath]) {
    ...
    if (!hasFound) {
      throw new PluginError('getFileInfo', 'not found ' + util.winPath(filepath) + ' of pkg ' + pkg.id);
    }
  }
...
function getStyleId (file, options) {
  ...
  return template(options.idleading, fileInfo.pkg)
    .replace(/\\/g, '/')
    .replace(/\/$/, '')
    .replace(/\//g, '-')
    .replace(/\./g, '_');
