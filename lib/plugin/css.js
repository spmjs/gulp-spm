...
var getFileInfo = require('../common').getFileInfo;
var join = path.join;
...
function parser(file, options) {
  ...
  function transportFile(file, pkg) {
    ...
    var fileObj = fInfo.pkg.files[fInfo.originPath];
    ...
    return imports(file.contents, function(item) {
      ...
      if (util.isRelative(dep)) {
        ...
        var depFile = fileObj.getDeps(dep);
        pkg_ = depFile.pkg;
        ...
        fullpath = join(pkg_.dest, depFile.path);
      }
