  ...
  function transportFile(file) {
    ...
    contents = resources(contents, function(item) {
      ...
      if (util.isRelative(dep)) {
        ...
        var depFileName = file.pkg.id.replace(/[@\.]/g, '_') + '_' + basename(dep);

        var f = new File({
          cwd: options.pkg.dest,
          base: options.pkg.dest,
          path: depFileName,
          contents: fs.readFileSync(depFilePath)
        });
        f.file = {
          path: join(options.pkg.dest, depFileName),
          pkg: options.pkg
        };
        f.path = join(options.pkg.dest, depFileName);
        self.push(f);

        return 'url('+depFileName+')';
      } else {
        return item.string;
      }
    });
